...
class SearchProductService {
  ...
  public async execute({ collection, productName}: IRequest): Promise<Product[]> {
    ...
    productsOfCollection = await this.productsRepository.findByCollectionName(collection);
    console.log(productsOfCollection)
    if((productsOfCollection.length > 0)) {
      
        if(productName && productName !== ""){
            products = productsOfCollection.filter(prod => prod.name.includes(productName))
        }
    } else {
